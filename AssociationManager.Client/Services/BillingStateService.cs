...
namespace AssociationManager.Client.Services
{
    public class BillingStateService
    {
        ...
        // Modal State
        public bool ShowCreateInvoiceModal { get; set; }
        public bool ShowTransactionHistory { get; set; }
        public bool ShowAdjustModal { get; set; }
        ...
        public async Task InitializeAsync()
        {
            try 
            {
                ...
                string paymentsUrl = "api/finance/payments";

                if (Scope == "association")
                {
                    var associationId = await GetAssociationIdAsync();
                    if (associationId != 0)
                    {
                ...
                var payments = await _api.GetAsync<IEnumerable<Payment>>(paymentsUrl);
                RecentPayments = payments?.ToList();

                AdvanceBalance = await _api.GetAsync<decimal>("api/finance/balance/asset");
                ...
                if (Scope == "association")
                {
                    ...
                    if (associationId != 0)
                    {
                        ExistingBatches = (await _api.GetAsync<IEnumerable<BillingBatch>>($"api/finance/batches?associationId={associationId}"))?.ToList();
