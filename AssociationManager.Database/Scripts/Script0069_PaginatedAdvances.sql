-- Script0069_PaginatedAdvances.sql
-- High-performance Server-Side Paging for Advances/Wallet History
...
BEGIN
    ...
    ;WITH FilteredAdvances AS (
        ...
        JOIN corp.Users u ON p.UserId = u.UserId
        JOIN assoc.Assets a ON p.AssetId = a.AssetId
        WHERE p.TenantId = @TenantId
          AND p.InvoiceId IS NULL -- This defines an "Advance" or "Wallet Top-up"
          ...
          AND (@SearchTerm IS NULL OR u.Name LIKE '%' + @SearchTerm + '%' OR a.Name LIKE '%' + @SearchTerm + '%')
    )
    SELECT 
        * 
