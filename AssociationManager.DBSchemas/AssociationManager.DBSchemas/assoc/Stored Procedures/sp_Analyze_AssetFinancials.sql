...
BEGIN
    ...
    SELECT 
        a.Name AS AssetName,
        (SELECT SUM(CASE WHEN Type = 'Debit' THEN Amount ELSE -Amount END) FROM assoc.Transactions WITH (NOLOCK) WHERE AssetId = @AssetId) AS CurrentBalance
    FROM assoc.Assets a WITH (NOLOCK) WHERE a.AssetId = @AssetId;
